const gorillaBattleRoster = [
  // Frontline (30)
 {
  name: "Dwayne 'The Rock' Johnson",
  role: "frontline",
  skills: ["wrestling", "acting", "intimidation"],
  task: "Leads the charge with overwhelming strength and charisma.",
  photo: "https://picsum.photos/300/300"
},
{
  name: "Shaquille O'Neal",
  role: "frontline",
  skills: ["basketball", "comedy", "size"],
  task: "Uses his size and humor to confuse and intimidate the gorilla.",
  photo: "https://picsum.photos/300/300"
},
{
  name: "Mike Tyson",
  role: "frontline",
  skills: ["boxing", "biting"],
  task: "Throws powerful punches and uses his biting tactics to fend off attacks.",
  photo: "https://example.com/photos/mike_tyson.jpg"
},
{
  name: "John Cena",
  role: "frontline",
  skills: ["wrestling", "acting", "strength"],
  task: "Distracts the gorilla with his famous catchphrase and wrestling moves.",
  photo: "https://example.com/photos/john_cena.jpg"
},
{
  name: "Jason Momoa",
  role: "frontline",
  skills: ["acting", "intensity", "fighting"],
  task: "Channels Aquaman’s fierceness to intimidate the beast.",
  photo: "https://example.com/photos/jason_momoa.jpg"
},
{
  name: "Terry Crews",
  role: "frontline",
  skills: ["acting", "strength", "enthusiasm"],
  task: "Charges with unstoppable energy and muscle power.",
  photo: "https://example.com/photos/terry_crews.jpg"
},
{
  name: "Chris Hemsworth",
  role: "frontline",
  skills: ["acting", "combat", "godly strength"],
  task: "Wields his 'Thor' hammer stance to intimidate and attack.",
  photo: "https://example.com/photos/chris_hemsworth.jpg"
},
{
  name: "Dave Bautista",
  role: "frontline",
  skills: ["wrestling", "acting", "muscle"],
  task: "Uses brute strength and wrestling skills to hold the line.",
  photo: "https://example.com/photos/dave_bautista.jpg"
},
{
  name: "Zion Williamson",
  role: "frontline",
  skills: ["basketball", "strength", "agility"],
  task: "Jumps and dodges with basketball agility to confuse the gorilla.",
  photo: "https://example.com/photos/zion_williamson.jpg"
},
{
  name: "Mark Wahlberg",
  role: "frontline",
  skills: ["acting", "boxing", "strength"],
  task: "Delivers sharp boxing combos to keep the gorilla off balance.",
  photo: "https://example.com/photos/mark_wahlberg.jpg"
},
{
  name: "Brock Lesnar",
  role: "frontline",
  skills: ["wrestling", "UFC", "intimidation"],
  task: "Uses MMA moves and aggression to dominate the fight.",
  photo: "https://example.com/photos/brock_lesnar.jpg"
},
{
  name: "Khabib Nurmagomedov",
  role: "frontline",
  skills: ["wrestling", "submission holds", "strength"],
  task: "Locks in submission holds to immobilize the gorilla.",
  photo: "https://example.com/photos/khabib_nurmagomedov.jpg"
},
{
  name: "Conor McGregor",
  role: "frontline",
  skills: ["boxing", "bravado", "trash talk"],
  task: "Taunts the gorilla to create openings for attack.",
  photo: "https://example.com/photos/conor_mcgregor.jpg"
},
{
  name: "Chuck Norris",
  role: "frontline",
  skills: ["martial arts", "strength", "legendary status"],
  task: "Uses legendary martial arts skills to intimidate and fight.",
  photo: "https://example.com/photos/chuck_norris.jpg"
},
{
  name: "Ronda Rousey",
  role: "frontline",
  skills: ["UFC", "judo", "strength"],
  task: "Throws precise judo throws to control the gorilla’s movements.",
  photo: "https://example.com/photos/ronda_rousey.jpg"
},
{
  name: "Hulk Hogan",
  role: "frontline",
  skills: ["wrestling", "entertainment", "strength"],
  task: "Rallies the team with loud cheers and powerful wrestling.",
  photo: "https://example.com/photos/hulk_hogan.jpg"
},
{
  name: "Ricky Gervais",
  role: "frontline",
  skills: ["comedy", "sarcasm", "quick wit"],
  task: "Distracts the gorilla with cutting sarcasm and wit.",
  photo: "https://example.com/photos/ricky_gervais.jpg"
},
{
  name: "Will Smith",
  role: "frontline",
  skills: ["acting", "martial arts", "charm"],
  task: "Combines charm and martial arts to confuse the gorilla.",
  photo: "https://example.com/photos/will_smith.jpg"
},
{
  name: "Jackie Chan",
  role: "frontline",
  skills: ["martial arts", "stunts", "speed"],
  task: "Uses acrobatic stunts and fast moves to evade attacks.",
  photo: "https://example.com/photos/jackie_chan.jpg"
},
{
  name: "Vin Diesel",
  role: "frontline",
  skills: ["acting", "strength", "determination"],
  task: "Uses sheer determination and strength to hold ground.",
  photo: "https://example.com/photos/vin_diesel.jpg"
},
{
  name: "Harrison Ford",
  role: "frontline",
  skills: ["acting", "marksmanship", "adventure"],
  task: "Shoots precision shots to scare the gorilla without harm.",
  photo: "https://example.com/photos/harrison_ford.jpg"
},
{
  name: "Gal Gadot",
  role: "frontline",
  skills: ["acting", "combat", "agility"],
  task: "Uses Wonder Woman’s combat skills to fend off the beast.",
  photo: "https://example.com/photos/gal_gadot.jpg"
},
{
  name: "Jason Statham",
  role: "frontline",
  skills: ["combat", "stunts", "toughness"],
  task: "Delivers tough fight moves to keep the gorilla at bay.",
  photo: "https://example.com/photos/jason_statham.jpg"
},
{
  name: "Tom Hardy",
  role: "frontline",
  skills: ["acting", "combat", "intensity"],
  task: "Channels intense focus to outmaneuver the gorilla.",
  photo: "https://example.com/photos/tom_hardy.jpg"
},
{
  name: "Michelle Rodriguez",
  role: "frontline",
  skills: ["acting", "fighting", "toughness"],
  task: "Uses street-fighting skills to engage the gorilla.",
  photo: "https://example.com/photos/michelle_rodriguez.jpg"
}
,
  {
    name: "Michelle Rodriguez",
    role: "frontline",
    skills: ["acting", "fighting", "toughness"],
    task: "Uses street-fighting skills to engage the gorilla.",
  },

  // Midline (30)
  {
    name: "Gordon Ramsay",
    role: "midline",
    skills: ["cooking", "yelling", "insults"],
    task: "Yells at the gorilla with such intensity it momentarily freezes.",
  },
  {
    name: "Joe Rogan",
    role: "midline",
    skills: ["commentary", "taekwondo", "podcasting"],
    task: "Provides live commentary to confuse the gorilla’s focus.",
  },
  {
    name: "Keanu Reeves",
    role: "midline",
    skills: ["acting", "martial arts", "being cool"],
    task: "Uses Matrix-style moves to dodge and distract.",
  },
  {
    name: "Ryan Reynolds",
    role: "midline",
    skills: ["acting", "comedy", "quick wit"],
    task: "Cracks jokes to lighten the mood and distract the gorilla.",
  },
  {
    name: "Jack Black",
    role: "midline",
    skills: ["comedy", "music", "acting"],
    task: "Performs comedic songs to confuse and charm.",
  },
  {
    name: "Will Ferrell",
    role: "midline",
    skills: ["comedy", "acting", "awkwardness"],
    task: "Uses awkward humor to disarm the gorilla.",
  },
  {
    name: "Kevin Hart",
    role: "midline",
    skills: ["comedy", "acting", "motivation"],
    task: "Motivates the team with energetic pep talks.",
  },
  {
    name: "Danny DeVito",
    role: "midline",
    skills: ["comedy", "acting", "being unpredictable"],
    task: "Distracts the gorilla with unpredictable antics.",
  },
  {
    name: "Post Malone",
    role: "midline",
    skills: ["music", "vibes", "chill"],
    task: "Creates chill vibes to calm tensions.",
  },
  {
    name: "Snoop Dogg",
    role: "midline",
    skills: ["rapping", "chilling", "strategizing"],
    task: "Raps to confuse and strategize the team’s moves.",
  },
  {
    name: "Chris Pratt",
    role: "midline",
    skills: ["acting", "comedy", "strength"],
    task: "Uses charm and strength to support frontline fighters.",
  },
  {
    name: "Tom Hanks",
    role: "midline",
    skills: ["acting", "emotional intelligence", "charm"],
    task: "Connects emotionally to calm the gorilla’s fury.",
  },
  {
    name: "Paul Rudd",
    role: "midline",
    skills: ["acting", "humor", "strength"],
    task: "Light-hearted humor to lighten the mood and confuse.",
  },
  {
    name: "Bruce Lee",
    role: "midline",
    skills: ["martial arts", "speed", "precision"],
    task: "Performs lightning-fast moves to distract and evade.",
  },
  {
    name: "Ben Stiller",
    role: "midline",
    skills: ["comedy", "acting", "impressions"],
    task: "Impersonates jungle animals to confuse the gorilla.",
  },
  {
    name: "Zach Galifianakis",
    role: "midline",
    skills: ["comedy", "improvisation", "awkwardness"],
    task: "Uses awkward improvisation to distract.",
  },
  {
    name: "Seth Rogen",
    role: "midline",
    skills: ["acting", "comedy", "vibes"],
    task: "Brings chill vibes and comedic relief.",
  },
  {
    name: "Jonah Hill",
    role: "midline",
    skills: ["comedy", "acting", "awkwardness"],
    task: "Uses awkward humor to diffuse tension.",
  },
  {
    name: "Charlie Day",
    role: "midline",
    skills: ["comedy", "acting", "chaos"],
    task: "Creates chaos to confuse the gorilla.",
  },
  {
    name: "Donald Glover",
    role: "midline",
    skills: ["acting", "music", "writing"],
    task: "Releases a surprise track called ‘This is Gorilla’ to distract.",
  },
  {
    name: "Emma Stone",
    role: "midline",
    skills: ["acting", "comedy", "adaptability"],
    task: "Pretends to be the gorilla’s long-lost love interest.",
  },
  {
    name: "Bo Burnham",
    role: "midline",
    skills: ["music", "comedy", "meta-awareness"],
    task: "Performs a musical breakdown about this ridiculous situation.",
  },
  {
    name: "Awkwafina",
    role: "midline",
    skills: ["acting", "comedy", "improv"],
    task: "Talks non-stop until the gorilla backs off.",
  },
  {
    name: "Ken Jeong",
    role: "midline",
    skills: ["comedy", "medicine", "chaotic energy"],
    task: "Diagnoses the gorilla with anger issues mid-dance.",
  },
  {
    name: "Lin-Manuel Miranda",
    role: "midline",
    skills: ["music", "writing", "performance"],
    task: "Drops an epic rap battle dissing the gorilla.",
  },
  {
    name: "Neil Patrick Harris",
    role: "midline",
    skills: ["acting", "magic", "hosting"],
    task: "Performs magic tricks to mystify the gorilla.",
  },
  {
    name: "Maya Rudolph",
    role: "midline",
    skills: ["comedy", "impressions", "singing"],
    task: "Impersonates jungle animals to confuse the gorilla.",
  },
  {
    name: "Donald Glover",
    role: "midline",
    skills: ["acting", "music", "writing"],
    task: "Creates distraction through surprise music performances.",
  },
  {
    name: "Awkwafina",
    role: "midline",
    skills: ["acting", "comedy", "improv"],
    task: "Talks nonstop to wear down the gorilla’s patience.",
  },
  {
    name: "Bo Burnham",
    role: "midline",
    skills: ["music", "comedy", "meta-awareness"],
    task: "Performs a comedic song about the situation.",
  },

  // Backline (40)
  {
    name: "Bill Nye",
    role: "backline",
    skills: ["science", "explanation", "teaching"],
    task: "Explains the gorilla’s behavior scientifically.",
  },
  {
    name: "Neil deGrasse Tyson",
    role: "backline",
    skills: ["astrophysics", "space", "education"],
    task: "Distracts with cosmic facts about the universe.",
  },
  {
    name: "Elon Musk",
    role: "backline",
    skills: ["engineering", "technology", "innovation"],
    task: "Builds a gorilla containment suit on the fly.",
  },
  {
    name: "Mark Rober",
    role: "backline",
    skills: ["engineering", "invention", "YouTube"],
    task: "Sets up gadgets and traps to outsmart the gorilla.",
  },
  {
    name: "MrBeast",
    role: "backline",
    skills: ["philanthropy", "challenges", "money"],
    task: "Offers the gorilla $1 million to surrender peacefully.",
  },
  {
    name: "Oprah Winfrey",
    role: "backline",
    skills: ["communication", "influence", "empathy"],
    task: "Convinces the gorilla through heartfelt conversation.",
  },
  {
    name: "Stephen Hawking",
    role: "backline",
    skills: ["physics", "theory", "communication"],
    task: "Calculates the safest way to subdue the gorilla.",
  },
  {
    name: "Marie Curie",
    role: "backline",
    skills: ["science", "radiation", "discovery"],
    task: "Creates a scientific shield to keep everyone safe.",
  },
  {
    name: "Ada Lovelace",
    role: "backline",
    skills: ["mathematics", "programming", "visionary"],
    task: "Programs AI drones to monitor gorilla movements.",
  },
  {
    name: "Carl Sagan",
    role: "backline",
    skills: ["astronomy", "science communication", "philosophy"],
    task: "Narrates the event to calm the gorilla’s mind.",
  },
  {
    name: "Jane Goodall",
    role: "backline",
    skills: ["primatology", "conservation", "empathy"],
    task: "Tries to bond and peacefully calm the gorilla.",
  },
  {
    name: "Tim Cook",
    role: "backline",
    skills: ["business", "technology", "leadership"],
    task: "Coordinates team logistics and resources.",
  },
  {
    name: "Sheryl Sandberg",
    role: "backline",
    skills: ["leadership", "communication", "strategy"],
    task: "Develops a communication strategy with the gorilla.",
  },
  {
    name: "Malala Yousafzai",
    role: "backline",
    skills: ["advocacy", "education", "peace"],
    task: "Advocates for peace and understanding with the gorilla.",
  },
  {
    name: "Greta Thunberg",
    role: "backline",
    skills: ["environmentalism", "activism", "passion"],
    task: "Calls out environmental destruction to shame the gorilla.",
  },
  {
    name: "Neil deGrasse Tyson",
    role: "backline",
    skills: ["astrophysics", "education", "space"],
    task: "Uses star charts to distract and calm the gorilla.",
  },
  {
    name: "Mark Zuckerberg",
    role: "backline",
    skills: ["technology", "social media", "innovation"],
    task: "Creates viral content to divert gorilla’s attention.",
  },
  {
    name: "Serena Williams",
    role: "backline",
    skills: ["tennis", "strength", "focus"],
    task: "Serves tennis balls precisely to occupy the gorilla.",
  },
  {
    name: "Stephen King",
    role: "backline",
    skills: ["writing", "horror", "imagination"],
    task: "Writes a terrifying story that scares the gorilla.",
  },
  {
    name: "J.K. Rowling",
    role: "backline",
    skills: ["writing", "fantasy", "world-building"],
    task: "Creates imaginary spells to confuse the gorilla.",
  },
  {
    name: "David Attenborough",
    role: "backline",
    skills: ["nature documentary", "voice narration", "knowledge"],
    task: "Narrates the gorilla’s behavior in calming tones.",
  },
  {
    name: "Tim Cook",
    role: "backline",
    skills: ["business", "technology", "leadership"],
    task: "Leads strategic retreats and resource allocation.",
  },
  {
    name: "Sheryl Sandberg",
    role: "backline",
    skills: ["leadership", "communication", "strategy"],
    task: "Keeps team morale high through speeches.",
  },
  {
    name: "Malala Yousafzai",
    role: "backline",
    skills: ["advocacy", "education", "peace"],
    task: "Promotes peaceful dialogue with the gorilla.",
  },
  {
    name: "Greta Thunberg",
    role: "backline",
    skills: ["environmentalism", "activism", "passion"],
    task: "Highlights the gorilla’s impact on the environment.",
  },
  {
    name: "Neil deGrasse Tyson",
    role: "backline",
    skills: ["astrophysics", "education", "space"],
    task: "Shares cosmic stories to distract.",
  },
  {
    name: "Mark Zuckerberg",
    role: "backline",
    skills: ["technology", "social media", "innovation"],
    task: "Creates distracting viral challenges.",
  },
  {
    name: "Serena Williams",
    role: "backline",
    skills: ["tennis", "strength", "focus"],
    task: "Uses tennis skills to occupy the gorilla.",
  },
  {
    name: "Stephen King",
    role: "backline",
    skills: ["writing", "horror", "imagination"],
    task: "Scares gorilla with a horror tale.",
  },
  {
    name: "J.K. Rowling",
    role: "backline",
    skills: ["writing", "fantasy", "world-building"],
    task: "Weaves magical distractions.",
  },
  {
    name: "David Attenborough",
    role: "backline",
    skills: ["nature documentary", "voice narration", "knowledge"],
    task: "Calm voice narration to soothe the gorilla.",
  },
  {
    name: "Elon Musk",
    role: "backline",
    skills: ["engineering", "technology", "innovation"],
    task: "Deploys tech gadgets to subdue the gorilla.",
  },
  {
    name: "Marie Curie",
    role: "backline",
    skills: ["science", "radiation", "discovery"],
    task: "Creates safe scientific barriers.",
  },
  {
    name: "Ada Lovelace",
    role: "backline",
    skills: ["mathematics", "programming", "visionary"],
    task: "Programs AI to predict gorilla moves.",
  },
  {
    name: "Carl Sagan",
    role: "backline",
    skills: ["astronomy", "science communication", "philosophy"],
    task: "Uses cosmic storytelling to calm the beast.",
  },
  {
    name: "Jane Goodall",
    role: "backline",
    skills: ["primatology", "conservation", "empathy"],
    task: "Bonds with the gorilla for peaceful resolution.",
  },
  {
    name: "Tim Cook",
    role: "backline",
    skills: ["business", "technology", "leadership"],
    task: "Organizes the team’s tech support.",
  },
  {
    name: "Sheryl Sandberg",
    role: "backline",
    skills: ["leadership", "communication", "strategy"],
    task: "Keeps morale high with motivating talks.",
  },
  {
    name: "Malala Yousafzai",
    role: "backline",
    skills: ["advocacy", "education", "peace"],
    task: "Advocates calm and peace.",
  },
  {
    name: "Greta Thunberg",
    role: "backline",
    skills: ["environmentalism", "activism", "passion"],
    task: "Calls attention to environmental respect.",
  },
  {
    name: "Neil deGrasse Tyson",
    role: "backline",
    skills: ["astrophysics", "space", "education"],
    task: "Distracts with star facts.",
  },
  {
    name: "Mark Zuckerberg",
    role: "backline",
    skills: ["technology", "social media", "innovation"],
    task: "Creates viral distractions.",
  },
  {
    name: "Serena Williams",
    role: "backline",
    skills: ["tennis", "strength", "focus"],
    task: "Uses precise tennis serves to distract.",
  },
];
